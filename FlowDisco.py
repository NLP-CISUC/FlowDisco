...
# Se 'True', qualquer speaker que não esteja em ENTIDADES_A_PROCESSAR será agrupado num único nó chamado "Outros".
...
# %%
# Validation
val = {"validation":False, "val_filename":"24_TREINO_new.xlsx", "val_filename_test":"24_TREINO_new.xlsx", "val_model":"all-MiniLM-L6-v2"}
...
# --- 4. Preparar Strings de Sentimento---
if tem_sentimento:
    # O round() nunca falha porque garantimos que são floats
    sod_visual = round(float(sod_val), 2)
    eod_visual = round(float(eod_val), 2)
    sent_variation = eod_visual - sod_visual
    
    str_fsc = f"Sentiment Cohesion: {fsc_atual:.2f}<br><br>"
    str_sent_flow = (
        f"Initial Sentiment: {sod_visual:.2f}<br>"
        f"Final Sentiment: {eod_visual:.2f}<br>"
        f"Sentiment Variation: {sent_variation:+.2f}<br>"
    )
else:
    str_fsc = "<br>"
    str_sent_flow = ""

# --- 5. Preparar Transition Coverage (Só mostra se > 0) ---
val_acc = globals().get('accuracy')
if val_acc is None: val_acc = globals().get('acc')

str_coverage = ""
# Verifica se é número E se é maior que zero
if isinstance(val_acc, (int, float)) and val_acc > 0:
    str_coverage = f"Transition Coverage: {val_acc:.2f}<br>"

# --- 6. Preparar F1 Score (Só mostra se > 0) ---
val_f1 = globals().get('ff1')
str_f1 = ""
if isinstance(val_f1, (int, float)) and val_f1 > 0:
    str_f1 = f"Flow F1-Score: {val_f1:.2f}<br>"

str_actions = ""
if actions_in_dataset > 0:
    str_actions = f"Actions in Dataset: {actions_in_dataset}<br>"

# Prepara a string das ações no grafo
str_actions_graph = ""
if n_action_nodes > 0:
    str_actions_graph = f'Number of Action States: <b id="count-action-states">{n_action_nodes}</b><br>'

# Definição dos Textos dos Tooltips
tips = {
    "utt_data": "Total count of speech turns (utterances) in the entire dataset.",
    "act_data": "Total count of non-verbal events (actions) recorded in the dataset.",
    "dial_data": "Number of distinct conversations/sessions processed.",
    "utt_per_dial": "Average number of turns per dialogue (Mean ± Standard Deviation).",
    "n_dial_states": "Total number of unique clusters (nodes) representing Dialogue States (Ellipses).",
    "n_act_states": "Total number of unique clusters (nodes) representing Action States (Boxes).",
    "sent_coh": "Flow Sentiment Cohesion (FSC): Measures consistency of sentiment within clusters.",
    "n_trans": "Total number of transitions (edges) between states in the graph.",
    "sent_ini": "Average sentiment of the start of conversations.",
    "sent_fin": "Average sentiment of the end of conversations.",
    "sent_var": "Difference between Final and Initial sentiment.",
    "density": "Graph Density: Ratio of existing edges vs. total possible edges.",
    "coverage": "Transition Coverage: Percentage of Test Set transitions found in this Graph.",
    "ff1": "Flow F1-Score: Balance between abstraction and precision."
}

# 1. Ações no Dataset (Dataset Metrics)
str_actions_dataset = ""
if actions_in_dataset > 0:
    str_actions_dataset = f'<span class="metric-help" title="{tips["act_data"]}">Actions in Dataset</span>: {actions_in_dataset}<br>'

# 2. Ações no Grafo (Clustering Metrics)
str_actions_graph = ""
if n_action_nodes > 0:
...
<div id="metrics-container" style="padding-left: 10px; font-size: 14px; line-height: 1.6;">
    ...
    <span class="metric-help" title="{tips["utt_per_dial"]}">Utterances per Dialogue</span>: {utterances_per_dialogue_mean:.1f}&nbsp;±&nbsp;{utterances_per_dialogue_std:.1f}<br><br>
    ...
    {str_actions_graph}
    <span class="metric-help" title="{tips["sent_coh"]}">Sentiment Cohesion</span>: {fsc_atual:.2f}<br><br>
    ...
    <span class="metric-help" title="{tips["sent_ini"]}">Initial Sentiment</span>: {sod_visual:.2f}<br>
    <span class="metric-help" title="{tips["sent_fin"]}">Final Sentiment</span>: {eod_visual:.2f}<br>
    <span class="metric-help" title="{tips["sent_var"]}">Sentiment Variation</span>: {sent_variation:+.2f}<br>

    <span class="metric-help" title="{tips["density"]}">Flow Density</span>: {density:.2f}<br>
    
    {str_coverage.replace("Transition Coverage", f'<span class="metric-help" title="{tips["coverage"]}">Transition Coverage</span>') if str_coverage else ""}
    {str_f1.replace("Flow F1-Score", f'<span class="metric-help" title="{tips["ff1"]}">Flow F1-Score</span>') if str_f1 else ""}
</div>
"""


# Prepara a string das ações no dataset (Dataset Metrics)
str_actions_dataset = ""
if actions_in_dataset > 0:
    str_actions_dataset = f"Actions in Dataset: {actions_in_dataset}<br>"
...
<div id="metrics-container" style="padding-left: 10px; font-size: 14px; line-height: 1.6;">
    ...
    Utterances per Dialogue: {utterances_per_dialogue_mean:.1f}&nbsp;±&nbsp;{utterances_per_dialogue_std:.1f}<br><br>

    <h3 style="margin-bottom: 5px;">Clustering Metrics</h3>
    Number of Dialogue States: <b id="count-states">{n_dialogue_nodes}</b><br>
    {str_actions_graph}
    {str_fsc}

    <h3 style="margin-bottom: 5px;">Flow Metrics</h3>
    Number of Transitions: <b id="count-edges">{n_edges}</b><br>
    {str_sent_flow}
    Flow Density: {density:.2f}<br>
    {str_coverage}{str_f1}
</div>
"""
...
<script>
    ...
    function enableNeighborHighlight(svgElement) {{
        ...
        nodes.forEach(node => {{
            node.addEventListener("mouseenter", () => {{
                ...
                edges.forEach(edge => {{
                    ...
                    if (source === hoveredName || target === hoveredName) {{
                        // Aresta Conectada -> Destacar
                        edge.classList.remove("graph-dimmed");
                        edge.classList.add("graph-highlight");
                        
                        // Guardar os nomes dos vizinhos
                        if (source === hoveredName) neighborNames.add(target);
                        if (target === hoveredName) neighborNames.add(source);
                        
                        // Pintar a seta de cor diferente se for saída ou entrada (Opcional)
                        const path = edge.querySelector("path");
                        if (path) {{
                            // Se sai do nó (Output) -> Verde, Se entra (Input) -> Azul (exemplo)
                            // path.style.stroke = (source === hoveredName) ? "green" : "#007bff";
                        }}
                    }}
                }});

                // 5. Destacar os Nós Vizinhos
                nodes.forEach(n => {{
            ...
            // 6. Reset ao tirar o rato
            node.addEventListener("mouseleave", () => {{
                nodes.forEach(n => {{
                    n.classList.remove("graph-dimmed", "graph-highlight");
                }});
                edges.forEach(e => {{
                    e.classList.remove("graph-dimmed", "graph-highlight");
                    // Reset da cor da seta se tiveres mudado
    ...
    function updateEdgeThickness(svgElement) {{
        svgElement.querySelectorAll("g.edge").forEach(edge => {{
            ...
            let prob = 0.1; // Valor base se não encontrar texto

            if (label) {{
                // Limpar vírgulas e converter para número
                const textVal = label.textContent.replace(",", ".");
                prob = parseFloat(textVal);
            }}

            // Proteção se a leitura falhar
            if (isNaN(prob)) prob = 0.1;

            // Base: 1px (para ser sempre visível)
            // Extra: Probabilidade * 5px
            // Exemplo: 
            //   Prob 0.1 -> 1 + 0.5 = 1.5px (Fina)
            //   Prob 1.0 -> 1 + 5.0 = 6.0px (Grossa)
            const thickness = 1 + (prob * 5);
            ...
            const polygon = edge.querySelector("polygon");
            if (polygon) {{
                 // Ajuste subtil para acompanhar a linha
    ...
    function applyFilters(threshold, speaker) {{
        ...
        // Se vier null ou undefined, assume "__all__"
        ...
        function getSpeakerName(fullName) {{
            ...
            // 2. Se tiver traço COM ESPAÇOS (" - "), corta (ex: PSD - Aplausos)
            // O "CDS-PP" mantém-se inteiro aqui!
        ...
        currentGraphElement.querySelectorAll("g.edge").forEach(edge => {{
            const label = edge.querySelector("text");
            // Se o valor tiver vírgula, troca por ponto para o parseFloat funcionar
            ...
            // Usamos a nova função inteligente para extrair os nomes
            ...
            if (allSpeakers) {{
                ...
                if (matchThreshold && (isFromSpeaker || isFromSOD || isToEOD)) {{
                    isActionable = true;
                }}
            }}
            ...
            if (isActionable) {{
                ...
                // Marcamos os nós de origem e destino como "ativos"
                nodesWithActiveEdges.add(sourceFull);
                nodesWithActiveEdges.add(targetFull);
            }} else {{
                edge.style.opacity = "0.05"; // Quase invisível
        ...
        currentGraphElement.querySelectorAll("g.node").forEach(node => {{
            const nodeName = node.getAttribute("data-full-name") || "";
            const nodeSpeaker = getSpeakerName(nodeName); // Usar a mesma lógica inteligente
            
            const isTargetSpeaker = !allSpeakers && (nodeSpeaker === speakerTarget);
            const isSpecial = (nodeSpeaker.toLowerCase() === "sod" || nodeSpeaker.toLowerCase() === "eod");
            ...
            if (shouldBeActive) {{
                ...
                node.style.opacity = "0.3"; // Esbatido
            }}
        }});
    ...
    function atualizarMetricasPorVisibilidade() {{
        setTimeout(() => {{
            ...
            const nodes = Array.from(currentGraphElement.querySelectorAll("g.node"));
            
            // 1. Filtrar apenas os visíveis
            const visibleNodes = nodes.filter(n => parseFloat(window.getComputedStyle(n).opacity) > 0.5);
            ...
            visibleNodes.forEach(node => {{
                ...
                // Graphviz: shape=box vira <polygon>, shape=ellipse vira <ellipse>
                ...
                // Sentimento
                let val = parseFloat(node.getAttribute("sentiment"));
                if (!isNaN(val)) {{
                    totalSent += val;
                    countSent++;
                }}
    ...
    if (searchInput) {{
        ...
        newInput.addEventListener("input", (e) => {{
            ...
            // 1. LIMPEZA TOTAL se a caixa estiver vazia
            if (query === "") {{
                // Removemos todas as classes de pesquisa e resets manuais
                currentGraphElement.querySelectorAll("g.node, g.edge").forEach(el => {{
                    el.classList.remove("faded", "hidden");
                    el.style.opacity = ""; // Remove opacidade forçada
                    
                    // Limpa a borda grossa da pesquisa
                    const shape = el.querySelector("ellipse, polygon, path");
                    if(shape) shape.style.strokeWidth = ""; 
                }});

                // Re-aplica os filtros normais (Speaker + Slider)
                applyFilters(threshold, currentSpeaker);
                return;
            }}
            ...
            currentGraphElement.querySelectorAll("g.node").forEach(node => {{
                ...
                if (fullText.includes(query)) {{
                    ...
                    // Destacar o encontrado
                    node.classList.remove("faded", "hidden");
                    node.style.opacity = "1";
                    
                    const shape = node.querySelector("ellipse, polygon, path");
                    if(shape) shape.style.strokeWidth = "5px"; // Borda grossa
                }} else {{
                    // Esconder o resto
            ...
            setTimeout(() => {{
                ...
                if (metricsDiv) {{
                    ...
                    <div style="font-size: 14px; line-height: 1.6; margin-top: 5px;">
                        ...
                        <span class="metric-help" title="Total number of unique clusters (nodes) representing Dialogue States.">Number of Dialogue States</span>: <b id="count-states">${{n_dialogue}}</b><br>
                        
                        ${{strActionsGraph ? strActionsGraph.replace("Number of Action States", '<span class="metric-help" title="Total number of unique clusters (nodes) representing Action States.">Number of Action States</span>') : ""}}
                        
                        <h3 style="margin-bottom: 5px; margin-top:10px;">Flow Metrics</h3>
                        <span class="metric-help" title="Total number of transitions (edges) between states in the graph.">Number of Transitions</span>: <b id="count-edges">${{n_edges}}</b>
                    </div>
    ...
    function renderNewDot(dotString) {{
        console.log("A iniciar renderizacao do grafo...");
        container.innerHTML = "";

        viz.renderSVGElement(dotString)
            .then(element => {{
                console.log("Grafo renderizado. A processar dados...");
                ...
                element.querySelectorAll("g.node").forEach(n => {{
                    ...
                    if(t) {{
                        ...
                        n.setAttribute("data-speaker", fullTitle.split("->")[0].trim().toLowerCase());
                ...
                element.querySelectorAll("g.edge").forEach(edge => {{
                    ...
                    if (title) {{
                        ...
                        if (sourceFull && targetFull) {{
                            ...
                            edge.setAttribute("data-source", sourceFull.split("->")[0].trim().toLowerCase());
                            
                            let targetLower = targetFull.split("->")[0].trim().toLowerCase();
                            if(targetLower.includes("(")) targetLower = targetLower.split("(")[0].trim();
                            edge.setAttribute("data-target", targetLower);
                        }}
                    }}
                }});

                makeNodesDraggable(element); 
                updateSpeakersFromGraph(element);
                updateLegendFromGraph(element);
